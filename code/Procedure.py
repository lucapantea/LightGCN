...
def BPR_train_original(dataset, recommend_model, loss_class, epoch, neg_k=1, w=None):
    ...
    for (batch_i, (batch_users, batch_pos, batch_neg)) \
            in tqdm(enumerate(utils.minibatch(users, posItems, negItems, batch_size=world.config['bpr_batch_size'])),
...
def Test(dataset, Recmodel, epoch, w=None, multicore=0):
    ...
    with torch.no_grad():
        ...
        for batch_users in tqdm(utils.minibatch(users, batch_size=u_batch_size),
                                desc='Validation', total=total_batch, leave=False):
