...
class AttentionLightGCN(BasicModel):

    def __init__(self, config: dict, dataset: BasicDataset):
        super(AttentionLightGCN, self).__init__()
        self.config = config
        self.dataset: BasicDataset = dataset
        self.embs = None
        self.__init_weight()
    ...
    def __init_weight(self):
        ...
        self.embedding_item = torch.nn.Embedding(
            num_embeddings=self.num_items, embedding_dim=self.latent_dim)
        self.attention_weights = torch.nn.Parameter(torch.randn(self.config['lightGCN_n_layers'] + 1),
                                                    requires_grad=True)
        if self.config['pretrain'] == 0:
            nn.init.normal_(self.embedding_user.weight, std=0.1)
            nn.init.normal_(self.embedding_item.weight, std=0.1)
        ...
        self.Graph = self.dataset.getSparseGraph()
        self.embs = None
        print(f"Attentive lgn is already to go(dropout:{self.config['dropout']})")
    ...
    def computer(self):
        ...
        # Splitting into user and item embeddings
        users, items = torch.split(light_out, [self.num_users, self.num_items])
        return users, items
...
class FinerAttentionLightGCN(BasicModel):

    def __init__(self, config: dict, dataset: BasicDataset):
        super(FinerAttentionLightGCN, self).__init__()
        self.config = config
        self.dataset: BasicDataset = dataset
        self.embs = None
        self.__init_weight()
    ...
    def __init_weight(self):
        ...
        self.embedding_item = torch.nn.Embedding(
            num_embeddings=self.num_items, embedding_dim=self.latent_dim)
        self.attention_weights_users = torch.nn.Parameter(torch.randn(self.config['lightGCN_n_layers'] + 1),
                                                          requires_grad=True)
        self.attention_weights_items = torch.nn.Parameter(torch.randn(self.config['lightGCN_n_layers'] + 1),
                                                          requires_grad=True)
        if self.config['pretrain'] == 0:
            nn.init.normal_(self.embedding_user.weight, std=0.1)
            nn.init.normal_(self.embedding_item.weight, std=0.1)
